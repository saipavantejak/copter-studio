...
describe('durable cloud execution',()=>{
 ...
 it('held-out evaluation freezes weights and uses disjoint seeds',()=>{const c=newTraining(config);advanceTraining(c,TRAIN_EPISODES);const q=JSON.stringify(c.q);advanceTraining(c,EVAL_PAIRS);expect(JSON.stringify(c.q)).toBe(q);expect(c.evaluations).toHaveLength(40);expect(c.evaluations.every(e=>e.baseline.seed>TRAIN_EPISODES)).toBe(true);expect(c.gate).toBe(evaluationGate(c.evaluations));expect(advanceTraining(c)).toBe(true);});
 ...
 it('gate rejects better reward if successful missions regress',()=>{const c=newTraining(config);advanceTraining(c,TRAIN_EPISODES+EVAL_PAIRS);const rows=c.evaluations.map(x=>({...x,baseline:{...x.baseline,successful:true,reward:-2},policy:{...x.policy,successful:false,reward:0}}));expect(evaluationGate(rows)).toBe(false);});
 it('native execution preserves held actions and training dynamics',()=>{const c=newTraining(config);advanceTraining(c,TRAIN_EPISODES+EVAL_PAIRS); // Synthetic gate fixture tests mechanics, not model quality.
