...
export const TestDashboard = ({ config, setConfig, tests, setTests, sensorCfg, setSensorCfg, telemetry }: TestDashboardProps) => {
  ...
  return (
    <div className="bg-surface border border-line rounded-xl p-4 flex flex-col h-full overflow-y-auto gap-5 text-sm">
      <div>
        <h2 className="text-base font-bold text-ink flex items-center gap-2 mb-3">
          <Settings className="w-4 h-4 text-emerald-700" /> Universal Architect
        </h2>

        <p className="text-xs text-amber-700 mb-3">Experimental model. Hardware inputs and simulated success are not independent flight validation.</p>
        <label className="block text-xs mb-2">Load reference aircraft
          ...
          }} className="w-full bg-surface p-2">
            <option value="">Choose a reference configuration</option>
            {AIRCRAFT_PROFILES.map(p=><option key={p.id} value={p.id}>{p.name}</option>)}
          </select>
        </label>
        <p className="text-xs text-amber-700">Propulsion: {propulsionEvidence(config).status}. Energy and dynamics remain approximate.</p>
        ...
        <details className="mb-3 text-xs">
          ...
          <textarea aria-label="Advanced aircraft configuration JSON" rows={8} value={configDraft} onChange={e=>setConfigDraft(e.target.value)}
            ...
            className="w-full bg-surface text-ink p-2 font-mono" />
          <p className="text-muted">JSON patch preserves unspecified fields. Units: batteryCapacity mAh, payloadMassKg kg, maxThrustPerMotorN N, inertiaOverride kg·m². propulsionCurve: ordered command/thrustN/powerW points spanning -1 to 1 at nominal voltage. Curves are not automatically validated.</p>
          ...
          {configError&&<p role="alert" className="text-red-700">{configError}</p>}
        </details>
        <div className="space-y-3">
          <div>
            <label className="block text-xs text-muted mb-1">Drone Type</label>
            <select value={config.droneType} onChange={e=>setConfig(editHardware(config,{droneType:e.target.value as DroneType}))}
              className="w-full bg-surface border border-line rounded-lg px-3 py-2 text-sm text-ink focus:outline-none focus:border-emerald-500">
          ...
          {/* Quick Presets */}
          <div>
            <label className="block text-xs text-muted mb-1.5">Quick Presets</label>
            <div className="flex flex-wrap gap-1.5">
              {[
                ...
                <button key={p.label} onClick={() => setConfig(c => editHardware(c,p.cfg))}
                  className="px-2 py-1 text-[10px] bg-canvas hover:bg-selected border border-line-strong rounded text-muted hover:text-ink transition-colors">
                  {p.label}
                </button>
              ))}
            </div>
          ...
          {[
            ...
            <div key={k}>
              <label className="block text-xs text-muted mb-1">{lbl}: <span className="text-ink">{(config as any)[k].toFixed(prec)}</span></label>
              ...
              <div className="text-[9px] text-muted mt-0.5">{hint}</div>
            </div>
          ))}
        </div>
      </div>
      ...
      {/* Test modules */}
      <div>
        <h3 className="text-xs font-bold text-muted uppercase tracking-wider mb-2">Test Modules</h3>
        <div className="space-y-2">
          {[
            ...
            <label key={k as string} className="flex items-center justify-between p-2.5 bg-surface rounded-lg border border-line cursor-pointer hover:border-line-strong transition-colors">
              <div className="flex items-center gap-2">
                {React.createElement(Icon as any, {className:`w-3.5 h-3.5 ${(tests as any)[k as string] ? (danger?'text-red-700':'text-emerald-700') : 'text-muted'}`})}
                <div>
                  <div className="text-xs text-ink">{lbl as string}</div>
                  <div className="text-[10px] text-muted">{sub as string}</div>
                </div>
              </div>
      ...
      <div>
        ...
        <label className="flex items-center justify-between p-2.5 bg-surface rounded-lg border border-line cursor-pointer hover:border-line-strong mb-2">
          <div>
            <div className="text-xs text-ink">Enable Sensor Noise</div>
            <div className="text-[10px] text-muted">IMU bias, baro drift, GPS jitter</div>
          </div>
        ...
        {sensorCfg.enableNoise && (
          <div className="space-y-2 pl-1">
            <div>
              <label className="block text-xs text-muted mb-1">IMU Noise: <span className="text-ink">{(sensorCfg.imuNoiseLevel*100).toFixed(0)}%</span></label>
            ...
            <div>
              <label className="block text-xs text-muted mb-1">GPS Noise: <span className="text-ink">{(sensorCfg.gpsNoiseLevel*100).toFixed(0)}%</span></label>
      ...
      {/* Mission presets */}
      <div>
        <h3 className="text-xs font-bold text-muted uppercase tracking-wider mb-2">Mission Preset</h3>
        <select value={tests.missionPreset} onChange={e=>setTests({...tests,missionPreset:e.target.value as any})}
          className="w-full bg-surface border border-line rounded-lg px-3 py-2 text-sm text-ink focus:outline-none focus:border-emerald-500 mb-3">
      ...
      <div className="p-3 bg-surface rounded-lg border border-line">
        <div className="flex items-center gap-2 mb-2">
          ...
          <span className="text-xs font-bold text-ink">Hover Precision</span>
        </div>
        <div className="grid grid-cols-2 gap-2 text-xs font-mono">
          <div className="bg-surface p-2 rounded border border-line">
            <div className="text-muted text-[10px] mb-0.5">GPS DRIFT</div>
            <div className={hoverMetrics.gpsDrift>0.5?'text-amber-700':'text-emerald-700'}>{hoverMetrics.gpsDrift.toFixed(3)} m</div>
          </div>
          <div className="bg-surface p-2 rounded border border-line">
            <div className="text-muted text-[10px] mb-0.5">ANG JITTER</div>
            <div className={hoverMetrics.angularJitter>0.1?'text-amber-700':'text-emerald-700'}>{(hoverMetrics.angularJitter*180/Math.PI).toFixed(2)}°</div>
          </div>
        </div>
      </div>
    </div>
