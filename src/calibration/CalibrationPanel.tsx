...
export function CalibrationPanel(){
 ...
 return <details className="rounded-xl border border-line-strong bg-surface p-4">
  ...
  <p className="text-xs text-amber-700 mt-2">The historical Bitcraze sweep uses varying supply voltage and whole-aircraft power. It cannot be applied as a constant-voltage per-motor calibration. No aircraft currently has verified battery or gust-endurance calibration here.</p>
  ...
  <p className="text-xs text-muted mt-2">Templates contain missing values deliberately. Supply real measurements before review. Files stay in this browser until you explicitly save a configuration through cloud history.</p>
  ...
  <label className="block text-sm mt-3">Calibration JSON<textarea aria-label="Calibration JSON" value={raw} rows={5} onChange={e=>{setRaw(e.target.value);setReview(null);setTrial(null);setNotice('');}} className="block w-full bg-surface border border-line-strong rounded p-2 text-xs font-mono" /></label>
  <button type="button" onClick={()=>inspect(raw)} className="mt-2 px-3 py-2 rounded bg-selected text-sm">Review measurements</button>
  ...
  {review&&<div className="text-sm mt-3 space-y-2">
   ...
   <button type="button" disabled={!review.passes||epRunning} onClick={()=>{try{setConfig(applyCalibration(review));setNotice('Imported bench calibration applied. Battery, wind and flight dynamics remain unvalidated.');}catch(e){setError((e as Error).message);}}} className="px-3 py-2 rounded bg-emerald-700 text-white disabled:opacity-40">Apply reviewed bench calibration</button>
  </div>}
