...
export function LearningPanel(){
 ...
 return <details className="rounded-xl border border-zinc-700 bg-zinc-900 p-4">
  ...
  <p className="text-sm text-zinc-300 my-3">80 measured points: one Crazyflie bench sweep and four propeller sweeps. Supervised regression and unsupervised operating-point clustering trained. 20 points withheld for interpolation checks; independent flight validation pending.</p>
  <p className="text-sm text-amber-400 mb-3">No measured gust-endurance data. Predictions apply only to the selected static test setup. The active simulator retains its existing physics and controller.</p>
  <div className="flex flex-wrap gap-3">
   <label className="text-sm">Measured setup <select aria-label="Measured setup" value={id} onChange={e=>{setId(e.target.value);const m=report.supervised.find(x=>x.id===e.target.value)!;setValue(String((m.range[0]+m.range[1])/2));}} className="block bg-zinc-800 p-2 max-w-full">{report.supervised.map(m=><option key={m.id} value={m.id}>{m.id}</option>)}</select></label>
   <label className="text-sm">{model.input} ({model.range.join('–')})<input aria-label="Measured operating point" type="number" value={value} onChange={e=>setValue(e.target.value)} className="block bg-zinc-800 p-2 w-36" /></label>
  </div>
  <p className="text-xs text-zinc-400 mt-2">{model.scope} <a href={model.source} target="_blank" rel="noreferrer" className="underline">Measurement source</a></p>
  <div role="status" className="my-3 text-sm">{error||Object.entries(prediction?.values??{}).map(([k,v])=>`${k}: ${v.toFixed(5)}`).join(' · ')}</div>
  <p className="text-xs text-zinc-400">Ct and Cp are dimensionless thrust and shaft-power coefficients. Displayed errors below are held-out residuals, not confidence intervals.</p>
  ...
  <div className="mt-4 border-t border-zinc-700 pt-3">
   ...
   <p className="text-xs text-amber-400 mt-2">Neural model did not beat the simpler regression. Nominal 90% interval covered {(energy.uncertainty.testCoverage*100).toFixed(1)}% of {energy.splitFlightIds.test.length} test flights. Onboard airflow is not ambient gust speed; cruise duration is not maximum endurance.</p>
