...
export function BenchmarkTab() {
  ...
  return (
    <main className="max-w-[1400px] mx-auto p-5 space-y-5">
        ...
        <div className="flex items-center gap-3 flex-wrap">
          ...
          <div className="flex items-center gap-2">
            ...
            <input aria-label="Benchmark seed" type="number" min={0} max={4294967295} step={1} value={masterSeed} onChange={e => { const n=e.target.valueAsNumber;if(Number.isSafeInteger(n)&&n>=0&&n<=4294967295)setMasterSeed(n); }}
              className="w-20 bg-surface border border-line rounded px-2 py-1 text-sm text-ink font-mono focus:outline-none focus:border-emerald-500" />
          </div>
          <div className="flex items-center gap-2">
            ...
            <select aria-label="Benchmark episodes" value={epNumEpisodes} onChange={e => setEpNumEpisodes(Number(e.target.value))}
              className="bg-surface border border-line rounded px-2 py-1 text-sm text-ink focus:outline-none focus:border-emerald-500">
      ...
      <p className="text-xs text-amber-700">Experimental simulation—not a validated aircraft digital twin. SEC is conditional on successful missions with explicit payload; failures remain in success rate and total energy.</p>
      <label className="text-xs text-muted">Benchmark duration (simulated seconds)
        <input type="number" min={1} max={3600} value={tests.mission?.durationSeconds ?? 16}
          ...
          className="ml-2 w-24 bg-surface border border-line-strong p-1" />
      </label>
      {batchStats && <p className="text-xs text-ink">Mission success: {((batchStats.successRate ?? 0)*100).toFixed(1)}% · Total energy across all attempts: {(batchStats.totalEnergyJ ?? 0).toFixed(1)} J · Valid SEC samples: {batchStats.efficiencySampleCount ?? 0}</p>}
      {batchStats?.successRate95CI && <p className="text-xs text-muted">95% interval for simulated mission success: {batchStats.successRate95CI.map(v=>(100*v).toFixed(1)+'%').join('–')}. Conditional on these test settings; not a real-flight reliability estimate.</p>}
      {/* Quick presets */}
      <div className="bg-surface border border-line rounded-xl p-4">
        <div className="flex items-center gap-2 mb-3">
          <Bookmark className="w-4 h-4 text-amber-700" />
          <h3 className="text-sm font-bold text-ink">Quick Presets</h3>
          <span className="text-[10px] text-muted">one-click benchmark configs</span>
        </div>
        <div className="flex flex-wrap gap-2">
          {[
            ...
            <button key={p.label} onClick={() => {
              ...
              title={p.desc}
              className="px-3 py-1.5 bg-canvas hover:bg-selected border border-line-strong text-ink rounded-lg text-[11px] font-medium transition-colors">
              {p.label}
            </button>
          ))}
        </div>
      ...
      <div className="bg-surface border border-line rounded-xl p-4">
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-2">
            ...
            <h3 className="text-sm font-bold text-ink">Domain Randomization</h3>
            <span className="text-[10px] text-muted">for sim-to-real robustness</span>
          </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4 text-xs">
            {[
              ...
              <div key={key as string}>
                <label className="text-muted mb-1 block">{lbl as string}: <span className="text-ink">
      ...
      {epRunning && (
        <div className="bg-surface border border-line rounded-xl p-4">
          <div className="flex justify-between text-sm mb-2">
            ...
            <span className="font-mono text-muted">{epProgress}/{epTotal}</span>
          </div>
      ...
      {batchStats && (
        <div className="flex gap-2">
          <button onClick={() => {
            ...
            className="px-3 py-1.5 bg-canvas hover:bg-selected border border-line-strong text-ink rounded-lg text-xs font-medium flex items-center gap-1.5 transition-colors">
      ...
      {epResults.length > 0 && (
        <div className="bg-surface border border-line rounded-xl overflow-hidden">
          <div className="px-4 py-3 border-b border-line flex items-center justify-between">
            ...
            <span className="text-xs text-muted">{epResults.length} episodes · seed={masterSeed}</span>
          </div>
            ...
            <table className="w-full text-xs font-mono">
              ...
              <tbody>
                {epResults.slice(-25).reverse().map(r => (
                  <tr key={r.id} className="border-b border-line/50 hover:bg-canvas/30">
                    <td className="px-2.5 py-1.5 text-muted">#{r.id + 1}</td>
                    <td className="px-2.5 py-1.5 text-muted text-[10px]">{r.seed.toString(16).slice(-6)}</td>
                    <td className="px-2.5 py-1.5">
                      <span className={`px-1.5 py-0.5 rounded text-[9px] font-bold ${r.controller === 'RL Policy' ? 'bg-emerald-500/20 text-emerald-700' : 'bg-amber-500/20 text-amber-700'}`}>
                        {r.controller === 'RL Policy' ? 'RL' : 'PD'}
                      </span>
                    </td>
                    <td className="px-2.5 py-1.5">
                      <span className={`px-1.5 py-0.5 rounded text-[9px] font-bold ${!r.successful ? 'bg-red-500/20 text-red-700' : 'bg-emerald-500/20 text-emerald-700'}`}>
                        {r.outcome ?? (r.crashed ? 'CRASH' : 'UNVERIFIED')}
                      </span>
                    </td>
                    <td className="px-2.5 py-1.5 text-ink">{r.survivalTime.toFixed(1)}s</td>
                    <td className="px-2.5 py-1.5 text-ink">{r.meanAltError.toFixed(3)}m</td>
                    <td className="px-2.5 py-1.5 text-ink">{(r.maxRoll * 180 / Math.PI).toFixed(1)}°</td>
                    <td className="px-2.5 py-1.5 text-ink">{(r.maxPitch * 180 / Math.PI).toFixed(1)}°</td>
                    <td className="px-2.5 py-1.5 text-amber-700">{r.successful && r.secApplicable ? r.sec.toFixed(4) : 'N/A'}</td>
                    <td className="px-2.5 py-1.5 text-blue-700">{r.spt.toFixed(4)}</td>
                    <td className="px-2.5 py-1.5 text-muted">{r.domainParams?.mass.toFixed(2)}kg</td>
                    <td className="px-2.5 py-1.5 text-muted">{r.domainParams?.motorTau ? (r.domainParams.motorTau * 1000).toFixed(0) + 'ms' : '—'}</td>
                  </tr>
                ))}
              </tbody>
            </table>
