...
function Bar({ value, min = -1, max = 1, color, label, unit, noisy }: {
  ...
  return (
    <div className="flex items-center gap-2 text-xs font-mono">
      <div className="w-14 text-zinc-500 shrink-0 text-right">{label}</div>
      <div className="flex-1 h-3 bg-zinc-800 rounded-sm relative overflow-hidden">
        ...
        {noisyPct !== undefined && (
          <div
            ...
            style={{ width: `${noisyPct}%`, background: '#f87171' }}
          />
        )}
        {/* True value */}
        <div
        ...
        />
        {/* Centre line */}
        <div className="absolute top-0 bottom-0 w-px bg-zinc-600" style={{ left: `${((0-min)/(max-min))*100}%` }} />
      </div>
      <div className="w-16 text-right" style={{ color }}>
        {value.toFixed(3)}<span className="text-zinc-600 text-[9px]"> {unit}</span>
      </div>
    </div>
  );
}
...
export const ObsActionViz: React.FC<ObsActionVizProps> = ({ state, action, noisy }) => {
  ...
  return (
    <div className="bg-zinc-900 border border-zinc-800 rounded-xl p-4 space-y-4 text-xs">
      ...
      <div>
        <div className="text-[10px] text-zinc-600 uppercase mb-1.5">Position</div>
        <div className="space-y-1">
          <Bar label="x" value={state.x} min={-5} max={5} color="#60a5fa" unit="m" noisy={noisy?.x} />
          <Bar label="y" value={state.y} min={-5} max={5} color="#60a5fa" unit="m" noisy={noisy?.y} />
          <Bar label="z" value={state.z} min={0} max={3}  color="#34a840" unit="m" noisy={noisy?.z} />
        </div>
      </div>

      {/* Observation — velocity */}
      <div>
        <div className="text-[10px] text-zinc-600 uppercase mb-1.5">Velocity</div>
        <div className="space-y-1">
      ...
      <div>
        <div className="text-[10px] text-zinc-600 uppercase mb-1.5">Attitude</div>
        <div className="space-y-1">
          <Bar label="φ" value={r2d(state.phi)}   min={-90} max={90} color="#fb923c" unit="°" noisy={noisy ? r2d(noisy.phi)   : undefined} />
          <Bar label="θ" value={r2d(state.theta)} min={-90} max={90} color="#fb923c" unit="°" noisy={noisy ? r2d(noisy.theta) : undefined} />
          <Bar label="ψ" value={r2d(state.psi)}   min={-180} max={180} color="#fb923c" unit="°" noisy={noisy ? r2d(noisy.psi) : undefined} />
        </div>
      </div>

      {/* Observation — angular rates */}
      <div>
        <div className="text-[10px] text-zinc-600 uppercase mb-1.5">Angular rates</div>
        <div className="space-y-1">
      ...
      {/* Action space */}
      <div>
        <div className="text-[10px] text-zinc-600 uppercase mb-1.5">
          Action (servos)
          {noisy && <span className="text-red-400 ml-1">· red = noisy obs</span>}
        </div>
        <div className="space-y-1">
      ...
      {state.motorOmegas && state.motorOmegas.length > 0 && (
        <div>
          <div className="text-[10px] text-zinc-600 uppercase mb-1.5">Motor ω (rad/s)</div>
          <div className="space-y-1">
            {state.motorOmegas.slice(0, state.droneType==='bicopter' ? 2 : state.droneType==='quadcopter' ? 4 : 6).map((omega,i) => (
              <Bar key={i} label={`M${i+1}`} value={omega} min={0} max={1200} color="#fbbf24" unit="r/s" />
            ))}
          </div>
        </div>
      )}
