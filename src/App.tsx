...
function AppContent() {
  ...
  return (
    <div className="min-h-screen bg-canvas text-ink font-sans selection:bg-emerald-500/30">
      {/* ── MAIN APPLICATION UI ──────────────────────────────────────── */}
      <div className={`flex flex-col h-screen transition-opacity duration-1000 ${(isStarted || isExiting) ? 'opacity-100' : 'opacity-0 pointer-events-none'}`}>
        <header className="border-b border-line bg-surface/50 backdrop-blur-xl sticky top-0 z-50">
          <div className="max-w-[1800px] mx-auto px-4 h-14 flex items-center justify-between gap-4">
            <div className="flex items-center gap-3 shrink-0">
              ...
              <div>
                <h1 className="font-bold text-sm tracking-tight text-ink">COPTER STUDIOS</h1>
                <p className="text-[9px] text-muted font-mono">The Premier Digital Twin for Autonomous Flight &amp; RL Research</p>
              </div>
            </div>
            {/* Tabs */}
            <div id="tab-bar" className="flex items-center gap-0.5 bg-surface border border-line rounded-lg p-1 overflow-x-auto responsive-tab-bar">
              {TABS.map(t => (
                <button key={t.id} onClick={() => setActiveTab(t.id)}
                  className={`flex items-center gap-1 px-2.5 py-1.5 rounded text-[11px] font-medium whitespace-nowrap transition-colors ${activeTab === t.id ? 'bg-selected text-ink' : 'text-muted hover:text-ink'}`}>
            ...
            <div className="flex items-center gap-3 text-xs font-mono text-muted shrink-0">
              {bestFlight && (
                <div className="hidden lg:flex items-center gap-1">
                  ...
                  <span className="text-amber-700 font-bold">{bestFlight.metrics.sec === 0 && bestFlight.metrics.hoverPowerW > 0
                    ? `Hover: ${bestFlight.metrics.hoverPowerW.toFixed(0)} W`
                    : bestFlight.metrics.sec.toFixed(3)}</span>
                </div>
              )}
              <span className="hidden md:inline">CTRL: <span className={controllerStatus === 'Loaded RL Agent' ? 'text-emerald-700' : 'text-amber-700'}>{controllerStatus}</span></span>
              <div className="flex items-center gap-1" title={crashData ? 'System Fault — the drone has crashed or exceeded safe flight limits' : 'System Nominal — all sensors and motors are functioning normally'}>
                <Activity className={`w-3 h-3 ${crashData ? 'text-red-700' : 'text-emerald-700 animate-pulse'}`} />
                <span className={crashData ? 'text-red-700 font-bold' : 'text-emerald-700'}>{crashData ? 'SYS_FAULT' : 'SYS_NOMINAL'}</span>
              </div>
              {/* Beginner's Guide — always one click away */}
              <button
                onClick={() => setShowGuide(true)}
                title="Open the Beginner's Guide"
                className="flex items-center gap-1 px-2 py-1 rounded-md border border-line-strong bg-surface hover:bg-emerald-500/10 hover:border-emerald-500/40 text-muted hover:text-emerald-700 transition-colors"
              >
